...
void LoginService::onGetQRCodeKeyFinished(error_code::ErrorCode code,
                                          const QByteArray& data) {
  if (code != error_code::ErrorCode::NoError) {
    emit qrCodeImageStatus(code);
    return;
  }
  QJsonDocument doc = QJsonDocument::fromJson(data);
  if (doc.isNull()) {
    emit qrCodeImageStatus(error_code::ErrorCode::JsonContentError);
    return;
  }
...
void LoginService::onCreateQRCodeImageFinished(error_code::ErrorCode code,
                                               const QByteArray& data) {
  if (code != error_code::ErrorCode::NoError) {
    emit qrCodeImageStatus(code);
    return;
  ...
  emit qrCodeImageStatus(error_code::ErrorCode::NoError);
  emit qrCodeImageData(img);
  checkQRCodeScanStatus();
}

void LoginService::onCheckQRCodeScanStatusFinished(error_code::ErrorCode code,
                                                   const QByteArray& data) {
  if (code != error_code::ErrorCode::NoError) {
    return;
  }
